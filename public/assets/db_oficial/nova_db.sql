...
--
-- Host: localhost
-- Tempo de geração: 10/05/2022 às 07:26
...
INSERT INTO `viagens` (`id`, `itinerario`, `data_partida`, `data_chegada`, `embarque_id`, `desembarque_id`, `veiculo_id`, `rota_id`, `classe_id`, `created_at`, `updated_at`) VALUES
(3, 'kikolo, cacuaco', '2022-05-10 05:20:43', '2022-05-10 08:00:00', 3, 4, 1, 2, 4, '2022-05-10 04:20:43', '2022-05-10 04:20:43');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `marca_modelos`  AS SELECT `marcas`.`id` AS `marca_id`, `marcas`.`marca` AS `marca`, `modelo`.`id` AS `modelo_id`, `modelo`.`modelo` AS `modelo` FROM (`marcas` join `modelos` `modelo` on(`marcas`.`id` = `modelo`.`marca_id`))  ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `viagem_detalhes`  AS SELECT `viagem`.`id` AS `id`, `rota`.`origem` AS `rota_origem`, `rota`.`destino` AS `rota_destino`, `rota`.`kilometros` AS `kilometros`, `rota`.`preco` AS `preco`, `viagem`.`itinerario` AS `itinerario`, `classes`.`classe` AS `classe`, `pontos_e`.`ponto` AS `ponto_e`, `pontos_d`.`ponto` AS `ponto_d`, `veiculo`.`matricula` AS `matricula`, `veiculo`.`marca` AS `marca`, `veiculo`.`modelo` AS `modelo`, `viagem`.`data_partida` AS `data_partida`, `viagem`.`data_chegada` AS `data_chegada`, `viagem`.`created_at` AS `created_at`, `viagem`.`updated_at` AS `updated_at` FROM (((((`viagens` `viagem` join `pontos_detalhes` `pontos_e` on(`viagem`.`embarque_id` = `pontos_e`.`id`)) join `pontos_detalhes` `pontos_d` on(`viagem`.`desembarque_id` = `pontos_d`.`id`)) join `veiculo_detalhes` `veiculo` on(`viagem`.`veiculo_id` = `veiculo`.`id`)) join `rotas` `rota` on(`rota`.`id` = `viagem`.`rota_id`)) join `classes` on(`viagem`.`classe_id` = `classes`.`id`))  ;
...
ALTER TABLE `viagens`
  ...
  ADD KEY `viagens_autocarro_id_foreign` (`veiculo_id`),
